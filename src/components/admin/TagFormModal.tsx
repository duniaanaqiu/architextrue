...
export function TagFormModal({ isOpen, onClose, initialData }: TagFormModalProps) {
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Only auto-update slug if we're creating new
    if (!initialData) {
      setSlug(
        newName
          .toLowerCase()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (initialData) {
        res = await updateTag(initialData.id, { name, slug });
      } else {
      ...
      <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            <label htmlFor="name" className="text-sm font-medium text-primary">Tag Name</label>
            <input
              id="name"
              type="text"
              value={name}
